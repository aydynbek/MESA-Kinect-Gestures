...
namespace KinectStreams
{
    ...
    public static class DataSignalSender
    {
        public static bool isClientConnected = false;
        private static Socket handler2;
        ...
        public static void SendSignal(string message)
        {
            ...
            {
                handler2.Send(Encoding.ASCII.GetBytes(message));
                canSendMessage = false;

                handler2.Receive(bytes);
                string receivedStr = Encoding.ASCII.GetString(bytes);

                if (receivedStr.IndexOf("SA") > -1)     // SA = Send Again
                {
                    canSendMessage = true;
                }
            }
